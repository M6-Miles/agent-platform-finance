...
> 重要提醒：本项目默认使用 Mock Agent 和内置样例数据，不调用真实大模型、不产生 API 费用。可选启用 DeepSeek、AkShare、腾讯证券公开行情，以及中国气象局 CMA-GRAPES 与 Open-Meteo 公开天气数据。模拟盘仅使用本地 MockBroker，不连接真实券商、不执行真实下单。所有分析仅供研究参考，不构成投资建议。
...
- **外部服务状态**：页面分别显示公开行情、天气 Provider 和 DeepSeek 的可用状态、最近成功时间、缓存与退避信息
...
天气 Provider 使用 5 分钟新鲜缓存和最多 6 小时的故障缓存。腾讯行情失败后仅在新浪公开
行情通过代码、价格、昨收、最高价和最低价一致性校验时切换备用源。
前端天气区域覆盖直辖市及新疆维吾尔自治区等省级行政区；只输入省级区域时会要求继续选择
具体城市，避免用一个省级坐标代表整个区域。中国境内联网预报优先使用 CMA-GRAPES，超出
CMA 可用时效后由 Open-Meteo Best Match 补全；每日结果明确显示模型来源。预置区县在地名
库缺少精确候选时使用省、市、区联合键约束的行政区中心参考坐标，未知区县才回退城市级。
证券分析接口同时校验六位 A 股代码和联网数据是否真实存在；例如 `600338` 可分析，`660338`
会返回明确的无效代码提示，不会复用其他股票的数据。
