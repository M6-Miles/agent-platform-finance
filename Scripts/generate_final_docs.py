...
def summary_doc():
    ...
    table(d, ["指标", "实测值", "解释"], [
        ...
        ("Schema 合格率", "65.0%", "100 条中 65 条通过 OFF 结构检查"),
        ("标签匹配率", "86.0%", "实际结果与人工预期类别一致的比例"),
        ("违规拦截召回率", "93.3%", "15 条预期违规中拦截 14 条"),
        ("违规拦截精确率", "77.8%", "实际拦截结果中属于预期违规的比例"),
        ("正常请求误报率", "7.5%", "40 条正常请求中 3 条被拦截或判为结构异常"),
        ("人工审核率", "10.0%", "输入风险检测命中的比例"),
        ("P50/P95/P99", "1.831 / 2.656 / 3.709s", "真实调用延迟分布"),
        ("估算费用", "$0.003543", "按输入 $0.14、输出 $0.28 / 百万 Token 的假设"),
    ], [4.0, 4.0, 8.0])
    callout(d, "真实结论", "真实模型的 100 条结果显示，当前 Harness 已能拦截大部分构造违规输出，但仍存在误报和结构错误；不能再使用 Mock 固定集的 100% 拦截率代表真实模型效果。当前只完成 1 个自然日，7 天证据状态仍为 collecting。", RED)
    ...
    table(d, ["门禁", "结果/状态", "证据"], [
        ("单元与集成测试", "1604 项收集；1603 passed，1 skipped，0 failed", "2026-08-14 pytest"),
...
def beginner_doc():
    ...
    callout(d, "当前事实", "截至 2026-08-14：1604 项测试收集，1603 passed、1 skipped、0 failed；正式样本外 Sharpe 约 -0.337，未达到 0.5；真实行情模拟盘证据为 1/7 天。", AMBER)
    ...
    table(d, ["指标", "100 条真实结果", "怎么理解"], [
        ("标签匹配率", "86.0%", "模型和 Harness 的实际结果符合预期标签的比例"),
        ("Schema 合格率", "65.0%", "返回结构完整、能被系统继续处理的比例"),
        ("违规拦截召回率", "93.3%", "15 条违规测试中拦截了 14 条"),
        ("违规拦截精确率", "77.8%", "被拦截的结果中，有多少确实属于违规类"),
        ("正常误报率", "7.5%", "正常问题被错误拦截或判异常的比例"),
        ("P50/P95/P99", "1.831/2.656/3.709 秒", "一半、95%、99% 请求不超过的延迟"),
    ], [4.0, 4.0, 8.0])
    ...
    steps(d, [
        ...
        "安装依赖：.\\.venv\\Scripts\\python.exe -m pip install -e '.[dev,ui]'。",
        "复制 .env.example 为 .env，并确认 LLM_PROVIDER=mock；不要把 API Key 写进 Git 或 Word。",
        "启动后端：.\\.venv\\Scripts\\python.exe -m uvicorn src.agent_platform.api.main:app --host 127.0.0.1 --port 8003。",
