...
| 用户级 Skill 市场 | 本地插件与 OpenAI/Anthropic 官方说明型 Skill 目录；按账号隔离，固定提交、Git 完整性校验和安装时 SHA256；外部工具依赖需另行接入 | 通过（说明型 Skill 不等于全部外部工具已接入） |
| 可观测图表 | 延迟、字符量和成功率按 Agent 汇总；短中文标签与窄屏旋转避免横轴重叠 | 通过 |
| 全量测试 | 1667 项收集，1666 passed，1 skipped，0 failed | 通过 |
