...
命令行 Demo 使用固定样例数据，用于验证 Harness 可移植性；Web 页面还提供联网天气模式。
中国境内联网预报优先使用中国气象局 `CMA-GRAPES`，超出其可用时效后由
`Open-Meteo Best Match` 补全。页面会为每个预报日显示模型来源，不把补全结果冒充 CMA。

区域选择按“省级行政区 - 城市 - 区县”组织。只选择新疆维吾尔自治区等省级区域时，
系统会要求继续选择具体城市；预置区县在地名服务缺少精确结果时使用所属省市约束的
参考中心坐标，未知区县才回退到城市级，并明确显示定位说明。
